...
### Данные
В приложении используются две сущности, которые описывают данные, — товар и покупатель: интерфейсы IProduct и IBuyer.
...
#### Интерфейс IBuyer
`payment: TPayment` - метод оптаты покупателя. В качетсве значений: 'card', 'cash' или ''.
`email: string` - электронная почта покупателя.
`phone: string` - номер телефона покупателя.
`address: string` - адрес покупателя.

### Модели данных
Для учёта данных в приложении есть три класса, которые разделены между собой по смыслу и зонам ответственности:
1. Products - хранение товаров, которые можно купить в приложении;
2. Basket - хранение товаров, которые пользователь выбрал для покупки;
3. Buyer - данные покупателя, которые тот должен указать при оформлении заказа. 
Для валидации используется специальный `type ValidError = Partial<Record<keyof IBuyer, string>>` - ключи соответствуют полям IBuyer, а значения - тексту ошибки.
...
`getProductById(id: string): boolean` - проверка наличия товара в корзине по его id, полученного в параметр метода.
...
Методы:
`setInfo(partialData: Partial<IBuyer>): void` - сохранение данных в модели;
`getInfo(): IBuyer` - получение всех данных покупателя;
`clear(): void` - очистка данных покупателя;
`isValid(): ValidError` - валидация данных.


### Слой коммуникации

#### Класс ApiQuery
Осуществляет связь приложения с сервером.

Конструктор:
`constructor(api: IApi)` - конструктор принимает объект, соответствующий интерфейсу IApi.
...
`catalog:changed` - изменение и вывод товаров
